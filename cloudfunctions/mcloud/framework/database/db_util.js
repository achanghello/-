...
async function getAllSinger(userId,collectionName, where, fields = '*', orderBy, size = MAX_RECORD_SIZE){

  size = Number(size);
  if (size > MAX_RECORD_SIZE) size = MAX_RECORD_SIZE;
  
  const blacklistUserIds  = await db.collection('bx_black').where({FAV_USER_ID:userId}).get({  
    fields: ['FAV_OID']  
  }).then(res => {  
    return res.data.map(item => item.FAV_OID);  
  });

  console.log("参数",blacklistUserIds);

  let query = await db.collection(collectionName)
  
  let findinfo = fmtWhere(where)

  if(blacklistUserIds.length>0){
    findinfo._id = db.command.nin(blacklistUserIds)
  }
  

	// 查询条件
	if (where)
    query = await query.where(findinfo);
...
async function getAll(collectionName, where, fields = '*', orderBy, size = MAX_RECORD_SIZE) {
  ...
  console.log("爱情公寓",fmtWhere(where));
	// 查询条件
	if (where)
		query = await query.where(fmtWhere(where));
