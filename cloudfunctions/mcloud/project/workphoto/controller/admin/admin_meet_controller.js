...
class AdminMeetController extends BaseProjectAdminController {
	...
	async insertMeet() {
		// await this.isAdmin();
		let rules = {
			phone: 'must|string|len:11|name=用户登陆手机'
		};
		// 取得数据
    let input = this.validateData(rules);
    ...
    let result = await service.insertMeet(this._openId,this._adminId, input);
    this.logOther('创建了新《' + input.title + '》');

    // 注册后登录一下下
    let service2 = new WorkHomeService();
    
    // 登陆改成用id登录
    return await service2.workLoginById(result.id);
    // return await service2.workLogin(input.phone, this._openId);
  ...
  async oneclicklogin() {
    ...
    this._request.phone = res;

    return this.insertMeet();
  }
	...
	async editMeet() {
		...
		let rules = {
			id: 'must|id',
			cateId: 'id|name=分类',
      cateName: 'string|name=分类',
      
			cancelSet: 'int|name=取消设置',
      daysSet: 'array|name=预约时间设置',
      
			phone: 'string|len:11|name=登陆手机',
			
			forms: 'array|name=表单',
      joinForms: 'array|name=用户资料设置',
