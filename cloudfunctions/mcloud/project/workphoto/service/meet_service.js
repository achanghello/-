...
class MeetService extends BaseProjectService {
    ...
    console.log("了不了",ret);
		let stat = { //统计数据
			succCnt: (ret['JOIN_STATUS_1'] || 0)+(ret['JOIN_STATUS_2'] || 0), //1=预约成功,
	...
	// 用户预约逻辑
	async join(userId, meetId, timeMark, formsList, address) {
    let user = await MeetModel.getOne({_id:userId})
		// 预约时段是否存在
		let meetWhere = {
			_id: meetId
		};
    ...
    data.JOIN_ADDRESS = address;
      ...
      //   JOIN_ID: joinId,
      //   JOIN_MEET_DAY: data.JOIN_MEET_DAY,
      //   JOIN_IS_CHECKIN: data.JOIN_IS_CHECKIN,
      //   JOIN_AVATAR: meet.MEET_OBJ.avatar,
      //   JOIN_MEET_STATUS: data.JOIN_STATUS,

      //   JOIN_MEET_NAME: meet.MEET_OBJ.name,
      //   JOIN_MEET_ADDRESS: data.JOIN_ADDRESS,
      // })
		}
    ...
    console.log("校验数据",userId, meetId, timeMark, formsList);
    ...
    whereJoin = {
		...
		let cntWork = await JoinModel.count(whereJoin);
		// 收藏人数
    ...
    ret.MEET_VIEW_CNT = meet.MEET_VIEW_CNT;  
		ret.cover = meet.MEET_OBJ.backgroundimage;
	...
	async detailForJoin(userId, meetId, timeMark) {
		...
		let fields = 'MEET_DAYS_SET,MEET_JOIN_FORMS, MEET_OBJ.avatar,MEET_OBJ.name';
    ...
    console.log("啦啦啦",where);
    let result = await MeetModel.getList(where, fields, orderBy, page, size, isTotal, oldTotal);
    
    for (let k = 0; k < result.list.length; k++) {
			where = {
				FAV_OID:result.list[k]._id
			};
			result.list[k].favedCnt = await FavModel.count(where);
			where = {
				FAV_OID:result.list[k]._id,
        FAV_USER_ID:userId
        
			};
			result.list[k].collected = await FavModel.count(where);
		}
    ...
    console.log("会不会难过",meet);
    if(meet.MEET_CANCEL_SET===0){
      this.AppError('歌手设置为不可取消');
    }
    ...
    let user = await MeetModel.getOne({_id: join.JOIN_USER_ID});

    // await JoinModel.del(where);
    await NewsModel.insert({
      JOIN_ID: joinId,
      JOIN_MEET_DAY: join.JOIN_MEET_DAY,
      JOIN_IS_CHECKIN: join.JOIN_IS_CHECKIN,
      JOIN_MEET_AVATAR: meet.MEET_OBJ.avatar,
      JOIN_USER_AVATAR: user.MEET_OBJ.avatar,
      JOIN_MEET_STATUS: 0,

      JOIN_MEET_NAME: meet.MEET_OBJ.name,
      JOIN_USER_NAME: user.MEET_OBJ.name,
      JOIN_MEET_ADDRESS: join.JOIN_ADDRESS,

      JOIN_USER_ID: join.JOIN_USER_ID,
      JOIN_MEET_ID: join.JOIN_MEET_ID,
    })

    await JoinModel.edit(joinId, { 	JOIN_STATUS: 0 });
    ...
    let fields = 'JOIN_COMPLETE_END_TIME,JOIN_IS_CHECKIN,JOIN_ADDRESS,JOIN_CHECKIN_TIME,JOIN_USER_ID,JOIN_USER_TITLE,JOIN_REASON,JOIN_MEET_ID,JOIN_MEET_TITLE,JOIN_MEET_DAY,JOIN_MEET_TIME_START,JOIN_MEET_TIME_END,JOIN_STATUS,JOIN_ADD_TIME,JOIN_CODE,JOIN_FORMS,JOIN_ADD_TIME';
    

		let where = {
			_id: joinId,
			// JOIN_USER_ID: userId  NewsModel
    };
    let join = await JoinModel.getOne(where, fields);
    // 查看
    console.log("参数",joinId,join.JOIN_MEET_ID,join.JOIN_USER_ID,userId);
    await this.editNew(joinId,join.JOIN_MEET_ID,join.JOIN_USER_ID,userId)

		return join
  }
  
  /**更新数据 */
	async editNew(JOIN_ID,JOIN_MEET_ID,JOIN_USER_ID,userId) {
    let data = await NewsModel.getList({JOIN_ID,JOIN_MEET_ID,JOIN_USER_ID});
    
    for (let i = 0; i < data.list.length; i++) {
      const element = data.list[i];
      console.log(456);
      if(userId===JOIN_MEET_ID){
        await NewsModel.edit(element._id, { JOIN_MEET_LOOK: 1 });
      }else if(userId===JOIN_USER_ID){
        console.log(456);
        await NewsModel.edit(element._id, { JOIN_USER_LOOK: 1 });
      }
    }
  ...
  async getNotices(userId,cateId){
    if(cateId===1){
      return await NewsModel.count({JOIN_MEET_ID:userId,JOIN_MEET_LOOK:0})
    }else if(cateId===2){
      return await NewsModel.count({JOIN_USER_ID:userId,JOIN_USER_LOOK:0})
    }else {
      return 0
    }
  }
	...
	async getMyJoinList(userId, {
		...
		let fields = 'JOIN_COMPLETE_END_TIME,JOIN_IS_CHECKIN,JOIN_USER_ID,JOIN_USER_TITLE,JOIN_REASON,JOIN_MEET_ID,JOIN_MEET_TITLE,JOIN_MEET_DAY,JOIN_MEET_TIME_START,JOIN_MEET_TIME_END,JOIN_STATUS,JOIN_ADD_TIME,JOIN_OBJ,JOIN_ADDRESS';
    ...
    console.log("哦排位哦五七九二哦i文件",where);
  ...
  /** 这里进行分类按日期搜索 */
  async getMyJoinListByDate(userId,phone,{date}){
    let user = MeetModel.getOne({_id: userId});
    let where = {};
    if(user.MEET_CATE_ID===2){
      where.JOIN_MEET_DAY=date;
      where.JOIN_STATUS=1;
      where.JOIN_USER_ID=user._id;
      where.JOIN_IS_CHECKIN=0;
    }else{
      where.JOIN_MEET_DAY=date;
      where.JOIN_STATUS=1;
      where.JOIN_MEET_ID=user._id;
      where.JOIN_IS_CHECKIN=0;
    }

    let result = await JoinModel.getList(where);
    return result;
  }
	...
	async getMyJoinSomeday(userId, day) {
  	...
  	/**更新数据 */
	async editJoin(id) {
    let data = await JoinModel.getOne({_id: id});
    let meet = await MeetModel.getOne({_id: data.JOIN_MEET_ID});
    let user = await MeetModel.getOne({_id: data.JOIN_USER_ID});
    await NewsModel.insert({
      JOIN_ID: id,
      JOIN_MEET_DAY: data.JOIN_MEET_DAY,
      JOIN_IS_CHECKIN: data.JOIN_IS_CHECKIN,
      ...
      JOIN_MEET_ID: data.JOIN_MEET_ID,
    })

    let result = await JoinModel.edit(id, { 	JOIN_STATUS: 1 });
    
    await this.statJoinCnt(data.JOIN_MEET_ID, data.JOIN_MEET_TIME_MARK);
