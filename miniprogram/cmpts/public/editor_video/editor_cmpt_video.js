...
Component({
    ...
    delete(e){
      ...
      let nodes = this.data.nodeList.filter((item,i)=>{
        return index!==i
      })
      this.setData({
        nodeList:nodes
		...
		bindAddImageTap: function (e) { 
			let that = this;
			wx.chooseMedia({
				count: 3-this.data.nodeList.length,
				mediaType: ['video'],
				sizeType: ['compressed'],
          ...
          wx.showLoading({
            title: '上传中',
					...
					for (let k = 0; k < res.tempFiles.length; k++) {
						...
						if (!contentCheckHelper.imgSizeCheck(size, imageMaxSize)) {
							wx.hideLoading();
							return pageHelper.showModal('视频大小不能超过 ' + maxSize + '兆');
						}
            ...
            let charBeforeDots = this.data.nodeList.map(item=>{
              const match = item.val.match(/(.*)[.]/);
              return Number(match[1].charAt(match[1].length - 1));
            })

            let key = 0;

            for (let i = 0; i < 3; i++) {
              if(charBeforeDots.includes(i)){
                continue;
              }
              key = i;
              break;
            ...
            let p = await wx.cloud.uploadFile({
              cloudPath: 'workphoto/meet/'+user._id+'/'+`video${key}`+ext,
              filePath: path, // 文件路径
            })

            let result = await cloudHelper.getTempFileURLOne(p.fileID)
      ...
      let videoCnt = 0;
        ...
        if (nodeList[k].type == 'video' && nodeList[k].val.trim() != '') videoCnt++;
			}
			if ((imgCnt + textCnt + videoCnt) == 0) {
				return [];
			}
